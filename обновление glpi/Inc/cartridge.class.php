...
class Cartridge extends CommonDBChild {
   ...
   static function showMassiveActionsSubForm(MassiveAction $ma) {
      ...
      switch ($ma->getAction()) {
          case 'uninstall' :
              $input = $ma->getInput();
              $er_occured = false;
              if (!isset($input['stockreturns_id'])) {
                  $input['stockreturns_id'] = '0';
              }
              if(isset($input['error_message'])){
                  $em = $input['error_message'];
                  switch($em){
                      case 1: $er_occured = true;
                          break;
                      case 2: $er_occured = true;
                          break;
                      case 3: $er_occured = true;
                          echo "<br>Местоположение возврата совпадает с местоположением картриджа!<br>";
                          break;
                  }
              }
              if($input['stockreturns_id'] == 0){
                  echo "<br>Место возврата не установлено! Выберите желаемое местоположение или обновите модель картриджа!<br>";
              }
              if(!$er_occured) {
                  StockReturn::dropdown(['value'  => $input['stockreturns_id']]);
   ...
   **/
   ...
   function post_updateItem($history = 1) {
      ...
      //достать locations_id из StockReturns (id для него будет в _POST или $this)
      //сделать $cart_item->update(), id для апдейта известен, locations_id подставить тот который достали из StockReturns();
      /*if(in_array('stockreturns_id', $this->updates)){
          $cart_item = new CartridgeItem();
          $cart_id = $this->fields['cartridgeitems_id'];
          if($cart_item->getFromDB($cart_id)){
              $stock_id = $cart_item->getField('stockreturns_id');
              $stock_ret = new StockReturn();
              if($stock_ret->getFromDB($stock_id)){
                  $loc_id = $stock_ret->getField('locations_id');
                  $cart_item->update(['id' => $cart_id,
                                      'locations_id' => $loc_id]);
                  Session::addMessageAfterRedirect(
                      __('Cartridge\'s locations has updated'),
                      true);
              }
          }
      }*/
   ...
   **/
            ...
            foreach ($ids as $key) {
               if ($item->can($key, UPDATE)) {
                  if ($item->uninstall($key) &&
                      $item->backToStockLoc($key, $input['stockreturns_id']) )
                  {
   ...
   function backToStockLoc($cID, $retId){
       global $DB;

       $stockreturns_id_query = "SELECT `locations_id`
                                 FROM `glpi_stockreturns`
                                 WHERE `id` = '$retId'";
       $resultReturntoStockId = $DB->query($stockreturns_id_query);
       if($DB->numrows($resultReturntoStockId)>0){
           $cart_location_id = $DB->result($resultReturntoStockId, 0, 0);
           $lId = $cart_location_id;
       } else {return false;}

       $IdQuery = "SELECT `cartridgeitems_id`
                  FROM `glpi_cartridges`
                  WHERE `id` = '$cID'";
       $resultId = $DB->query($IdQuery);
       if ($DB->numrows($resultId)>0) {
           $cart_item_id = $DB->result($resultId, 0, 0);
           $cId = $cart_item_id;}
           else {return false;}

       if($cId>0){
       //TODO : timestamp() в текстовое поле!
       $query = "UPDATE `glpi_cartridgeitems`
                SET `locations_id`='".$lId."'
                WHERE `id` = '".$cId."'";
       if ($result = $DB->query($query)
           && ($DB->affected_rows() > 0)) {
           $changes[0] = '0';
           $changes[1] = '';
           $changes[2] = __('Возврат картриджа в кабинет, конец срока службы');
           Log::history($this->getField("cartridgeitems_id"), 'CartridgeItem', $changes,
   ...
   **/
   function install($pID, $tID, array $input) {
      global $DB;

       $retID = $input['stockreturns_id'];
   ...
   **/
   static function showForPrinter(Printer $printer, $old = 0) {
      ...
      if ($canedit && $number) {
         ...
         ///////////////////////поиск stockreturns_id в таблице сartridgeitems по id принтера
         $cart = new Cartridge();
         if($cart->getFromDB($printer->fields['id'])){
             $cart_id = $cart->getField('id');//id of cartridge
             $cart_item = new CartridgeItem();
             if($cart_item->getFromDB($cart_id)){
                 $cart_item_stockloc = $cart_item->getField('stockreturns_id');
                 $cart_item_loc = $cart_item->getField('locations_id');
                 $error_message = 0;
                 $st_ret = new StockReturn();
                 if($st_ret->getFromDB($cart_item_stockloc)){
                     $st_ret_loc = $st_ret->getField('locations_id');
                     if($cart_item_loc == $st_ret_loc){
                         $error_message = 3;
                     }
                 }
         ...
         $massiveactionparams = ['num_displayed'    => min($_SESSION['glpilist_limit'], $number),
                           ...
                           'extraparams'      => ['maxpages'
                                                  ...
                                                  'stockreturns_id' => $cart_item_stockloc,
                                                  'error_message'
                                                        => $error_message
                                                 ]
                            ];
